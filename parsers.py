            ...
            for i in range(len(time_left)):
                while time_left[i] == 0:
                    ...
                    if isinstance(evt, midi.NoteEvent):
                        if (evt.get_pitch() >= self.__lowest_pitch) or (evt.get_pitch() <= self.__highest_pitch):
                            if isinstance(evt, midi.NoteOffEvent) or evt.get_velocity() == 0:
                                state[evt.get_pitch()-self.__lowest_pitch] = 0                      
                            else:
                                state[evt.get_pitch()-self.__lowest_pitch] = 1   
                        pass
                    try:
